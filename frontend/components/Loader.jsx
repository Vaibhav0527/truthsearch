...
export default function Loader({ onDone }) {
  ...
  useEffect(() => {
    let sTimer;
    const scramble = () => { setTitle([..."TRUTHLENS"].map(c => Math.random() > .6 ? chars[Math.floor(Math.random() * chars.length)] : c).join("")); sTimer = setTimeout(scramble, 60); };
    scramble();
    // Count up — 6-8s total duration
    const iv = setInterval(() => {
      setCount(p => {
        const next = Math.min(p + Math.floor(Math.random() * 2 + 1), 100);
        if (next >= 100) {
          clearInterval(iv); clearTimeout(sTimer); setTitle("TRUTHLENS");
          setTimeout(() => { setPhase("out"); setTimeout(() => { setGone(true); onDone(); }, 1100); }, 400);
        }
        return next;
      });
    }, 65);
  ...
  return (
    <div style={{
      ...
      transition: phase === "out" ? "clip-path 1.1s cubic-bezier(0.76,0,0.24,1)" : "none",
    }}>
      {/* Grid overlay */}
      ...
      <div style={{ position: "relative", textAlign: "center", zIndex: 2 }}>
        ...
        <div style={{ width: 320, height: 2, background: "rgba(181,123,255,.1)", borderRadius: 2, margin: "0 auto", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: 0, left: 0, height: "100%", width: `${count}%`, background: "linear-gradient(to right,#b57bff,#6d28d9)", boxShadow: "0 0 16px rgba(181,123,255,.8)", borderRadius: 2, transition: "width .065s linear" }} />
        </div>
      ...
      {/* Scan line */}
      <div style={{ position: "absolute", left: 0, right: 0, height: 1, background: "linear-gradient(to right,transparent,rgba(181,123,255,.4),transparent)", top: `${count}%`, transition: "top .065s linear", pointerEvents: "none" }} />
    </div>
  );
}
