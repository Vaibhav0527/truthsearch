...
// ─── AI DETECT PAGE ──────────────────────────────────────────────────────────
export default function AIDetectPage({ t, setPage }) {
  ...
  const handleFile = (e) => {
    ...
    if (f) { setImg(URL.createObjectURL(f)); setImgFile(f); setResult(null); setError(null); }
  };
  ...
  const handleDownload = () => downloadReport({
    inputType: "AI Image Detection",
    userInput: "[Image uploaded for AI detection]",
    result,
    isAi: true,
  });
    ...
    <div style={{ paddingTop: 84, minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ borderRight: `1px solid ${t.line}`, padding: "52px 48px", overflowY: "auto", maxHeight: "calc(100vh - 68px)" }}>
        ...
        {img && (
          ...
          >
            {analyzing ? "Analyzing Forensics…" : "Detect AI Image"}
          </Btn>
        )}
        ...
        {analyzing && (
          <div style={{ marginTop: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 11, color: t.muted, letterSpacing: ".1em" }}>AI analyzing image…</span>
            </div>
            <Scanner t={t} />
      ...
      <div ref={rRef} style={{ padding: "52px 48px", overflowY: "auto", maxHeight: "calc(100vh - 68px)" }}>
        ...
        {!result && !analyzing && !error && (
          <div style={{ textAlign: "center", paddingTop: 100, color: t.faint }}>
            ...
            <p style={{ marginTop: 16, fontFamily: "'Space Grotesk',sans-serif", fontSize: 15, lineHeight: 1.7 }}>
              Upload an image to detect<br />if it's AI-generated or real
            </p>
          </div>
        )}
        ...
        {result && (
          <div>
            ...
            <TiltCard t={t} style={{
              ...
              <h4 style={{
                ...
                <span style={{
                  ...
                  background: result.verdict === "AI-Generated" ? t.lo + "15" : result.verdict === "Real Photograph" ? t.hi + "15" : t.mid + "15",
                  color: result.verdict === "AI-Generated" ? t.lo : result.verdict === "Real Photograph" ? t.hi : t.mid,
                  border: `1px solid ${result.verdict === "AI-Generated" ? t.lo + "35" : result.verdict === "Real Photograph" ? t.hi + "35" : t.mid + "35"}`,
                }}>
            ...
            {result.visual_inconsistencies && result.visual_inconsistencies.length > 0 && (
              <TiltCard t={t} style={{
                padding: 24, marginBottom: 16,
                opacity: rVis ? 1 : 0, transform: rVis ? "translateY(0)" : "translateY(28px)",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {result.visual_inconsistencies.map((item, i) => (
                    <div key={i} style={{
                      display: "flex", alignItems: "flex-start", gap: 10,
            ...
            {result.ai_generation_indicators && result.ai_generation_indicators.length > 0 && (
              <TiltCard t={t} style={{
                padding: 24, marginBottom: 16,
                opacity: rVis ? 1 : 0, transform: rVis ? "translateY(0)" : "translateY(28px)",
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {result.ai_generation_indicators.map((ind, i) => (
                    <span key={i} style={{
